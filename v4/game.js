class Game{
	constructor(){
		...
		const options = {
			assets:[
                ...
                `${this.assetsPath}fbx/key.fbx`,
                `${this.assetsPath}fbx/bau.fbx`,
			],
			oncomplete: function(){
                game.lighting(1);
				game.init();
        ...
        if (event.keyCode == 80) {
            ...
            if (event.keyCode == 76) {
                if(this.iluminacao == 0){
                    this.iluminacao = 1;
                    
                    game.light.intensity = 0.5; 
		...
		if (this.onAction.mode !== undefined){
			switch(this.onAction.mode){
                ...
                case 'collect':
                    ...
                    this.collect[this.cont].visible = false;
                    //document.write(this.onAction.index);
                    if (this.collected==undefined) this.collected = [];
                    this.collected.push(this.cont);
                    
                    var img = document.createElement("IMG");
                    img.src = "usb.jpg"
                    document.getElementById("briefcase").children[0].children[0].appendChild(img).children[0];
                    //document.getElementById("briefcase").appendChild(img);
                    
                    if (this.cont == 3){
                        document.write("<img src='win.jpg'>")
                    }
	...
	init() {
        ...
        alert('Objetivo: Colete 3 chaves e depois leve-as ao báu.');
		// ground
      ...
      loadBau(loader, x, y, z){
		const game = this;  
		
		loader.load( `${this.assetsPath}fbx/bau.fbx`, function ( object ) {
            game.scene.add(object);
            const scale = 0.25;
    ...
    loadKey(loader, x, y, z){
		const game = this;
		
		loader.load( `${this.assetsPath}fbx/key.fbx`, function ( object ) {
            const scale = 0.15;
			object.scale.set(scale, scale, scale);
			object.name = "key";
            object.position.set(x, y, z);
	...
	loadEnvironment(loader){
		...
		loader.load( `${this.assetsPath}fbx/environment.fbx`, function ( object ) {
			...
			game.loadKey(loader, -404, 1.5, -641);
            game.loadKey(loader, -1802, 14, 474);
            game.loadKey(loader, -1834, 1.5, -1589);
            game.loadBau(loader,2201,15,-78);
		}, null, this.onError );
	}
